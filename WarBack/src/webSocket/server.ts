...
io.on("connection", (socket: Socket) => {
  ...
  socket.on("launchAttack", async (attack: IAttack) => {
    try {
      ...
      const targetName: string | null =
        newAttack.target instanceof Organization ? newAttack.target.name : null;
      const sourceName: string | null =
        newAttack.source instanceof Organization ? newAttack.source.name : null;
  ...
  socket.on("updateAttack", async (attack: IAttack) => {
    try {
      ...
      const targetName: string | null =
        updatedAttack.target instanceof Organization
          ? updatedAttack.target.name
          : null;
      const sourceName: string | null =
        updatedAttack.source instanceof Organization
          ? updatedAttack.source.name
          : null;
      if (targetName === null || sourceName === null) {
        throw new Error("Target or source not found");
      }
